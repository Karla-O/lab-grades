---
title: "Second graded lab"
author: "Karla Otoude"
format: html
---
```{r}
library(here)

here::i_am("lab-grades.Rproj")
```

```{r}
library(tidyr)
library(dplyr)
library(ggplot2)
library(vroom)
library(readr)
library(lubridate)
```

# Link to the github repository
https://github.com/Karla-O/lab-grades.git

# 1. Introduction 

## 1.1 Study organisation

### Question 1
```{r}
courses <- vroom(here("courses.csv"),
                col_types = cols(
    nb_grades = col_integer(),
    module = col_factor())) |> 
  rename(nb_exams = nb_grades)
```



### Question 2 
```{r}
courses |>
  summarise("course name" = course, 
            "identifier" = course_id,
            "module" = module,
            "number of exams" = nb_exams) |>
  arrange(courses$course) |>
  knitr::kable()
```

## 1.2 Students

### Question 3
```{r}
students <- vroom(here("students.csv"),
                col_types = cols(
    group = col_factor(), 
    id = col_integer(), 
    sex = col_factor()))
```
The `birth_date` column of the students data frame is of class `Date`.


## 1.3 Grades

### Question 4
```{r}
grades <- vroom(here("grades.csv"),
                     delim = ";", 
                     na = "unknown",
                col_types = cols(
                  id = col_integer(),
                  course_id = col_integer()
                ))
```


# 2. Student population analysis

### Question 5
```{r}
students |>
  ggplot(aes(x = group)) +
  geom_bar() + 
  labs(
    title = "Number of students per group",
    x = "Group",
    y = "Number of students"
  )
```


### Question 6
```{r}
students |>
  ggplot(aes(x = group, fill = sex)) +
  geom_bar() + 
  labs(
    title = "Gender representation in each group",
    x = "Group",
    y = "Number of students"
  )
```

### Question 7
```{r}
students |>
  mutate(age = round(time_length(today()-birth_date, unit="year"))) |>
  ggplot(aes(x = age)) +
  geom_histogram(binwidth = 1, fill = 'blue', color = "black") +
  labs(
    title = "Distribution of age among students",
    x = "Age",
    y = "Number of students"
  )
```

### Question 8
```{r}
students |>
  mutate(
    age = round(as.numeric((today() - birth_date) / dyears(1)))
  ) |>
  summarise(
    med_st = median(age, na.rm = TRUE),
   .by = group) |>
  arrange(group) |>
  knitr::kable()
```

### Question 9
```{r}
students |>
  mutate(
    "age" = round(as.numeric((today() - birth_date) / dyears(1)))
  ) |>
  filter(age == max(age), .by = group) |>
  select(id, sex, age, group) |>
  arrange(desc(age)) |>
  knitr::kable()
```


# Simple grade analysis

### Question 10
```{r}
nb_grades <- grades |> 
  filter(!is.na(grade)) |> 
  nrow()
```

The data set contains `r nb_grades`  grades 


### Question 11
```{r}
grades_per_course <- 
  left_join(grades, courses, by = "course_id")
all_info <- 
  left_join(grades_per_course, students, by = "id")

all_info |>
  filter(course == "Martial Arts and Self-Defense") |>
  group_by(group) |>
  summarise(mean_MASD = mean(grade, na.rm = TRUE)) |>
  ggplot(aes(x = group, y = mean_MASD)) +
  geom_col()
```

### Question 12
```{r}
all_info |>
  filter(!is.na(grade)) |>
  ggplot(aes(x = grade, fill = module)) +
  geom_histogram(position = "dodge", bins = 20) +
  labs(
    title = "Students results depending on their module",
    x = "grade obtained",
    y = "nb of grades"
  )
```

# Attendance analysis
### Question 13
```{r}
grades |>
  group_by(id) |>
  summarise(nb_of_grades = n()) |>
  slice_head(n=5) |>
  knitr::kable()
```

### Question 14
```{r}
all_info |>
  select(id, grade, sex) |>
  group_by(id, sex)|>
  summarise(nb_of_grades = n(), sex) |>
  distinct(.keep_all = TRUE) |>
  ggplot(aes(x = nb_of_grades, fill = sex)) +
  geom_histogram(position = "dodge", bins = 20) +
  labs(
    title = "Distribution of the number of grades depending on sex",
    x = "nb of grades the students have",
    y = " "
    
  )
```


### Question 15
```{r}
all_info |>
  filter(course == "Music and Dance") |>
  group_by(id, group) |>
  summarise("Music grade" = n(), .groups = "drop") |>
  slice_head(n = 5) |>
  knitr::kable()
```


### Question 16
```{r}
grades_music_dance <- all_info |>
  filter(course == "Music and Dance") |>
  group_by(id, group) |>
  summarise(`Music grade` = n(), .groups = "drop")

grades_music_dance |>
  group_by(`Music grade`) |>
  summarise(effectif = n()) |>
  ggplot(aes(x = `Music grade`, y = effectif)) +
  geom_col() +
  labs(
    title = "Distribution for each grade in Music and Dance",
    x = "Number of grades in Music and Dance",
    y = "Number of students"
  )
```



### Question 17
```{r}
grades_music_dance |>
  ggplot(aes(x = as.factor(group), y = `Music grade`)) +
  geom_jitter(width = 0.2, alpha = 0.7, color = "pink") +
  labs(
    title = "Distribution of the number of grades per group",
    x = "group",
    y = "Number of grades in music and dance"
  )
```

